            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <input type="button" value="+ mahasiswa" class="btn btn-primary" onclick=location.href="{{ route('admin.tambah')}}" title="Add User" style="margin-left: 20px;">
                        <!-- /.panel-heading -->
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                @if(count($dataMahasiswa)>0)
                                    @foreach ($dataMahasiswa as $angs)
                                <tr align="center">
                                    <td>
                                        @if (!empty($angs->nama_photo))
                                            <img src="{{ asset('uploads/products/' . $angs->nama_photo) }}" 
                                            alt="{{ $angs->nama }}" width="50px" height="50px">
                                        @else
                                             <img src="http://via.placeholder.com/50x50" alt="{{ $angs->nama_photo }}">
                                         @endif
                                    </td>
                                    @if($angs->nama==null)
                                    ...
                                    <td>ga ada data<div align="center"></div></td>
                                     @else
                                        <td>{{ $angs->umur}}<div align="center"></div></td>
                                    @endif
                                    @if($angs->nim==null)
                                    <td>ga ada data<div align="center"></div></td>
                                     @else
                                        <td>{{ $angs->nim}}<div align="center"></div></td>
                                    @endif
                                    @if($angs->konsentrasi==null)
                                    <td>ga ada data<div align="center"></div></td>
                                     @else
                                        <td>{{ $angs->konsentrasi}}<div align="center"></div></td>
                                    @endif
                                    
                                     <td bgcolor="#EEF2F7"><a href={{URL('midtest/editMahasiswa')}}/{{$angs->id}}>Edit</a>|
                                     <a href="{{ URL('midtest/hapusMahasiswa')}}/{{$angs->id}}">hapus</a></div></td>
